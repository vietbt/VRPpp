...
def plot_solution(nodes, tours, title=None):
    fig = go.Figure()
    fig.update_layout(width=320, height=240)
    ...
    fig.add_trace(go.Scatter(x=pos_x_2, y=pos_y_2, mode='lines', name=name, line_color=colors[1], line_width=1))
    
    for pos_x, pos_y, name in all_pos:
        fig.add_trace(go.Scatter(x=pos_x[1:-1], y=pos_y[1:-1], mode='lines', name=name, line_color=colors[2], line_width=1))
    ...
    if len(station_x) > 1:
        fig.add_trace(go.Scatter(x=station_x, y=station_y, mode='markers', name="station", marker_color="black", marker_size=4, marker_symbol="square"))
    fig.add_trace(go.Scatter(x=[depot.x], y=[depot.y], mode='markers', name="depot", marker_color="red", marker_size=8, marker_symbol="hexagram"))
