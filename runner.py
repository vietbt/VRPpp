...
class Runner:
    ...
    def random_choice(self, data):
        ...
        actions = [np.random.choice(ids, 3, replace=False) for ids in mask_ids]
        return actions
    ...
    def run(self):
        ...
        for step in range(self.n_steps):
            ...
            if self.policy is not None:
                ...
                actions = self.random_choice(obs)
                values, log_probs = None, None
        ...
        if self.policy is None or not self.policy.training:
            return True
